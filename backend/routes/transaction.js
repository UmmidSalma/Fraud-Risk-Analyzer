...
router.post('/process', async (req, res) => {
    ...
    db.get('SELECT * FROM Users WHERE user_id = ?', [req.user_id], async (err, user) => {
        ...
        const status = risk.level === 'High' ? 'PAUSED_OTP' : risk.level === 'Moderate' ? 'PAUSED_OTP' : 'COMPLETED';
        ...
        function(errT) {
            ...
            if (status === 'PAUSED_OTP') {
                // Send OTP for both High and Moderate risk
                const otp = Math.floor(100000 + Math.random() * 900000).toString();
                const message = risk.level === 'High' ? 'High fraud risk detected. Verify via OTP to proceed.' : 'Unusual transaction detected. Verify via OTP.';
                console.log(`[SYS-MAIL] TXN Verification OTP for ${user.email} (TXN ${txnId}): ${otp}`);
                console.log(`[SYS-SMS] TXN Verification OTP for ${user.mobile} (TXN ${txnId}): ${otp}`);
                return res.json({ txnId, status, risk_score: risk.score, risk_level: risk.level, message, otp_required: true, server_mock_otp: otp });
            } 
            else {
