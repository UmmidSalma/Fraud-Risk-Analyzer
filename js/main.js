...
const app = {
   ...
   init() {
       ...
       this.adminUsers = [];
       this.adminUserPendingFreezeEmail = null;
   ...
   startOtpCountdown(type, seconds = 60) {
       ...
       const updateTimer = () => {
           ...
           if (el) el.textContent = `${String(minutes).padStart(2, '0')}:${String(secs).padStart(2, '0')}`;
           if (remaining <= 0) {
               this.stopOtpCountdown(type);
               if (btn) {
                   btn.disabled = false;
   ...
   async resendTransactionOtp() {
       ...
       try {
           const token = localStorage.getItem('token');
            const response = await fetch(`${API_BASE_URL}/api/transaction/send-otp`, {
               method: 'POST',
               headers: { 
               ...
               body: JSON.stringify({
                   email: (this.pendingTransaction?.email || localStorage.getItem('userEmail') || '').toLowerCase(),
                   transactionId: this.pendingTransaction?.txnId || `TXN${Date.now()}`,
                   mobile: this.pendingTransaction?.mobile || null,
                   pendingTransaction: {
       ...
       if(!txnPin || txnPin.length !== 4) return alert("Transaction PIN must be 4 digits.");
        ...
        try {
            ...
            if(res.ok) {
                ...
                this.startOtpCountdown('reg', 60);
                console.log('📱 OTP sent to email and mobile for registration');
            } else {
                alert(data.error || data.message || 'Unable to send OTP');
            }
        } catch (err) { console.warn('Registration OTP failed', err); alert('Server not running or config error.'); }
   },
   ...
   async fetchWithAuth(url, options = {}) {
       ...
       try {
           ...
           console.warn('fetchWithAuth: response not JSON', jsonErr);
       }
       const payload = (body && typeof body === 'object')
           ? (body.data || body.transactions || body.users || body)
           : body;
       return { ok: res.ok, data: payload, error: body.error || body.message || res.statusText, status: res.status };
   },
   ...
   async loadAdminUsers() {
       ...
       if(res.ok) {
           ...
           this.adminUsers.forEach(u => {
               ...
               const statusClass = u.status === 'BLOCKED' ? 'danger-c' : 'neon';
               tbody.innerHTML += `<tr><td>${u.email}</td><td class="${roleCol}">${u.role}</td><td class="${statusClass}">${u.status}</td><td><button class="scan-btn mini" onclick="app.openAdminUserDetails('${u.email}')">DETAILS</button> <button class="scan-btn mini warn" onclick="app.openAdminUserFreeze('${u.email}')">FREEZE</button></td></tr>`;
           });
       }
   },
   ...
   openAdminUserDetails(email) {
       ...
       if (!user) return alert('Unable to locate user details');
       document.getElementById('adminUserDetailsName').textContent = user.name || 'Unknown';
       document.getElementById('adminUserDetailsEmail').textContent = user.email;
       document.getElementById('adminUserDetailsMobile').textContent = user.mobile || 'Unknown';
       document.getElementById('adminUserDetailsAge').textContent = this.getAge(user.dob);
       document.getElementById('adminUserDetailsDob').textContent = user.dob || 'N/A';
       document.getElementById('adminUserDetailsCity').textContent = user.city || 'N/A';
       document.getElementById('adminUserDetailsCreatedAt').textContent = user.createdAt ? new Date(user.createdAt).toLocaleString() : 'N/A';
       document.getElementById('adminUserDetailsStatus').textContent = user.status || 'ACTIVE';
       document.getElementById('adminUserDetailsReason').textContent = user.freezeReason || 'None';
       document.getElementById('adminUserDetailsModal').style.display = 'flex';
   },

   openAdminUserFreeze(email) {
       console.log('[ADMIN] openAdminUserFreeze called for', email, 'pending users:', this.adminUsers?.length);
       const user = (this.adminUsers || []).find((u) => u.email === email);
       if (!user) {
           console.warn('[ADMIN] openAdminUserFreeze could not find user', email);
           return this.showAdminUserMessage('Unable to locate user details for freeze.', 'error');
       }
       this.adminUserPendingFreezeEmail = email;
       document.getElementById('adminFreezeTargetEmail').textContent = email;
       document.getElementById('adminFreezeReason').value = 'Invalid logins';
       document.getElementById('adminFreezeOtherReasonRow').style.display = 'none';
       this.clearAdminUserModalMessage();
       document.getElementById('adminUserFreezeModal').style.display = 'flex';
   },

   closeAdminUserModal() {
       const detailsModal = document.getElementById('adminUserDetailsModal');
       const freezeModal = document.getElementById('adminUserFreezeModal');
       if(detailsModal) detailsModal.style.display = 'none';
       if(freezeModal) freezeModal.style.display = 'none';
   },

   async submitAdminUserFreeze() {
       console.log('[ADMIN] submitAdminUserFreeze called, target=', this.adminUserPendingFreezeEmail);
       const reasonSelect = document.getElementById('adminFreezeReason');
       const otherInput = document.getElementById('adminFreezeOtherReason');
       let reason = reasonSelect ? reasonSelect.value : '';
       if (reason === 'Other') {
           reason = otherInput ? otherInput.value : '';
       }
       if (!reason) return this.showAdminUserModalMessage('Please choose or enter a freeze reason.', 'error');
       if (!this.adminUserPendingFreezeEmail) return this.showAdminUserModalMessage('Missing target user.', 'error');

       try {
           const requestUrl = `${API_BASE_URL}/api/admin/users/freeze`;
           console.log('[ADMIN] freeze request URL:', requestUrl);
           console.log('[ADMIN] freeze request body:', { email: this.adminUserPendingFreezeEmail, reason });

           const res = await fetch(requestUrl, {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
               body: JSON.stringify({ email: this.adminUserPendingFreezeEmail, reason }),
           });
           ...
           try {
               ...
               console.warn('[ADMIN] freeze response not JSON', jsonErr);
               body = { message: responseText };
           }

           console.log('[ADMIN] freeze response status:', res.status, res.statusText, body);

           if (res.ok) {
               this.showAdminUserMessage(body?.message || 'User blocked successfully', 'success');
               this.showAdminUserModalMessage(body?.message || 'User blocked successfully', 'success');
               this.closeAdminUserModal();
               this.loadAdminUsers();
           } else {
               const errorText = body?.error || body?.message || res.statusText || 'Unknown error';
               console.error('Admin freeze failed:', res.status, errorText);
               this.showAdminUserModalMessage(errorText, 'error');
           }
       } catch (err) {
           console.error('Admin freeze exception:', err);
           this.showAdminUserModalMessage('Unable to freeze user: ' + (err.message || 'Unknown error'), 'error');
       }
   },

   showAdminUserMessage(message, variant = 'error') {
       const el = document.getElementById('adminUserMessage');
       if (!el) return;
       el.innerHTML = `<span class="title">${variant === 'success' ? 'Success' : 'Error'}</span><span>${message}</span>`;
   ...
   showAdminUserModalMessage(message, variant = 'error') {
       const el = document.getElementById('adminFreezeModalMessage');
       if (!el) return this.showAdminUserMessage(message, variant);
       el.innerHTML = `<span class="title">${variant === 'success' ? 'Success' : 'Error'}</span><span>${message}</span>`;
       el.classList.toggle('success', variant === 'success');
       el.style.display = 'block';
   },

   clearAdminUserModalMessage() {
       const el = document.getElementById('adminFreezeModalMessage');
       if (!el) return;
       el.style.display = 'none';
       el.innerHTML = '';
   ...
   onAdminFreezeReasonChange() {
       const reasonSelect = document.getElementById('adminFreezeReason');
       const otherRow = document.getElementById('adminFreezeOtherReasonRow');
       if (reasonSelect && otherRow) {
           otherRow.style.display = reasonSelect.value === 'Other' ? 'block' : 'none';
       }
   },
   ...
   async initiateTransaction() {
    ...
    try {
        ...
        if (risk.includes('HIGH') || risk.includes('MODERATE')) {
            ...
            let otpCode = null;
            try {
                // Send transaction to backend to get OTP
                const token = localStorage.getItem('token');
                const txnId = `TXN${Date.now()}`;
                this.pendingTransaction.txnId = txnId;
                const txnResponse = await fetch(`${API_BASE_URL}/api/transaction/send-otp`, {
                    method: 'POST',
                    headers: { 
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                ...
                if (txnResponse.ok) {
                    const txnData = await txnResponse.json().catch(() => ({}));
                    otpCode = txnData.server_mock_otp || txnData.otp || null;
                    if (txnData.transactionId) {
                        this.pendingTransaction.txnId = txnData.transactionId;
                    }
                    console.log('📱 OTP sent to registered email and mobile for transaction verification');
                    console.log('Mock OTP for testing (check server console for SMS):', otpCode);
                }
            } catch (otpErr) {
                console.warn('Transaction OTP request failed:', otpErr);
            }
            if (!otpCode) {
  ...
  // Prefer server-side OTP verification for correctness
  try {
      const token = localStorage.getItem('token');
      const txnId = this.pendingTransaction.txnId || `TXN${Date.now()}`;
    const res = await fetch(`${API_BASE_URL}/api/transaction/verify-otp`, {
          method: 'POST',
      ...
      if (res.ok && data.success) {
          alert('✅ OTP verified. Transaction completed.');
          this.resetTxnForm();
          this.loadUserHistory();
          this.loadUserDashboard();
          this.navigate('user-transaction-history');
      ...
      // If server rejected OTP, block the txn
      ...
      alert(data.message || 'Invalid OTP. Transaction blocked.');
      ...
      console.warn('OTP verification request failed, falling back to local check', err);
  }

  // Fallback: local expected OTP (for dev/testing only)
  const expected = (this.pendingTransaction.expectedOtp || this.expectedOtp || '').toString().trim();
  if (!expected) return alert('OTP verification unavailable. Please retry.');
  if (otp !== expected) return alert('Wrong OTP entered.');
...
if (risk_score > 80) {
    ...
    await fetch(`${API_BASE_URL}/api/transaction/send-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            email: (this.userEmail || localStorage.getItem('userEmail') || '').toLowerCase(),
            transactionId: this.pendingTransaction?.txnId || `TXN${Date.now()}`,
            mobile: this.pendingTransaction?.mobile || null,
            pendingTransaction: {
                amount: this.pendingTransaction?.amount,
